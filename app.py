...
def generate_offer_pdf(candidate, hr_user, pattern='standard'):
    ...
    if pattern == 'custom' and custom_body:
        ...
        body_text = (
            ...
            f'<b>\u20b9{salary}</b>, which encompasses base salary, statutory benefits, '
...
def letterhead_preview_html(hr_user, candidate, pattern='standard'):
    ...
    if lh_path and os.path.exists(lh_path):
        ...
        <div style="margin-bottom:16px">
          <div style="background:#fff8e1;border:1px solid #f59e0b;border-radius:8px;
                      padding:10px 16px;margin-bottom:10px;font-size:12px;color:#92400e;display:flex;align-items:center;gap:8px">
            <span style="font-size:16px">📄</span>
            <div><b>Company Letterhead attached</b> — PDF will be attached to the offer email sent to the candidate</div>
          </div>
          <iframe src="data:application/pdf;base64,{lh_b64}" width="100%" height="160"
    ...
    rows = ''.join(
        ...
        f'<td style="padding:10px 14px;border:1px solid #e2e8f0;font-weight:700;color:#1a56db;width:42%;font-size:12px">{k}</td>'
    ...
    # Build body text per pattern
    if pattern == 'custom' and custom_body:
        body_html = f'<p style="font-size:12px;color:#475569;line-height:1.8;margin-bottom:14px">{custom_body}</p>'
    elif pattern == 'formal':
        body_html = f'''<p style="font-size:12px;color:#475569;line-height:1.8;margin-bottom:14px">
            ...
            the organisation, including but not limited to the Code of Conduct, Non-Disclosure Agreement,
            and applicable labour laws.
        </p>
        <p style="font-size:12px;color:#475569;line-height:1.8;margin-bottom:14px">
            The candidate is required to report to duty on the date specified herein, failing which this
            offer shall stand null and void. All documents as required shall be submitted on or before
            the date of joining.
        </p>'''
    elif pattern == 'modern':
        body_html = f'''<p style="font-size:13px;color:#0d1b3e;line-height:1.9;margin-bottom:14px;font-weight:600">
            Hey {name} — we're absolutely thrilled to have you on board! 🎉
        </p>
        <p style="font-size:12px;color:#475569;line-height:1.8;margin-bottom:14px">
            You'll be joining us as <b style="color:#1a56db">{role}</b> starting <b>{joining}</b>.
            We move fast, think big, and we believe you're exactly the kind of person who fits right in.
            We can't wait to see what you'll build with us.
        </p>'''
    elif pattern == 'detailed':
        body_html = f'''<p style="font-size:12px;color:#475569;line-height:1.8;margin-bottom:14px">
            We are pleased to offer <b>{name}</b> the position of <b style="color:#1a56db">{role}</b>
            at <b>{company}</b>. Your compensation package has been carefully designed to recognise
            your experience and potential contribution to the organisation.
        </p>
        <div style="background:#f8faff;border-radius:8px;padding:14px 16px;margin-bottom:14px;border-left:3px solid #1a56db">
            <div style="font-size:11px;font-weight:700;color:#0d1b3e;margin-bottom:8px;text-transform:uppercase;letter-spacing:.05em">Compensation Breakdown</div>
            <div style="display:flex;justify-content:space-between;font-size:11.5px;color:#475569;margin-bottom:4px"><span>Annual CTC</span><span style="font-weight:700;color:#1a56db">₹ {salary}</span></div>
            <div style="display:flex;justify-content:space-between;font-size:11px;color:#94a3b8"><span>Includes base salary, PF, gratuity &amp; applicable allowances</span></div>
        </div>'''
    else:  # standard
        body_html = f'''<p style="font-size:12px;color:#475569;line-height:1.8;margin-bottom:14px">
            ...
            experience are an excellent fit for our team, and we look forward to the contributions
            you will make.
        </p>'''

    # Pattern-specific header style
    if pattern == 'modern':
...
<div style="font-family:Georgia,serif;background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden">
  <div style="padding:24px 28px">
    ...
    <div style="{header_style}padding:24px 28px;border-radius:8px;margin-bottom:20px">
      <div style="font-family:Arial,sans-serif;font-size:20px;font-weight:900;color:#fff;text-align:center">{company}</div>
      <div style="font-size:10px;color:rgba(255,255,255,.65);margin-top:3px;text-align:center">Human Resources Department</div>
      <div style="margin-top:12px;padding-top:12px;border-top:1px solid rgba(255,255,255,.2);text-align:center;font-size:14px;font-weight:700;color:#fff;letter-spacing:.08em">{title_text}</div>
    </div>
    <div style="font-size:10px;color:#64748b;margin-bottom:12px">Date: {today}</div>
...
<table width="580" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08)">
  <tr><td style="background:linear-gradient(135deg,#0d1b3e 0%,#1a56db 100%);padding:32px 40px;text-align:center">
    <div style="width:64px;height:64px;background:rgba(255,255,255,.15);border-radius:50%;margin:0 auto 16px;display:flex;align-items:center;justify-content:center;font-size:28px">🛡️</div>
    <div style="font-size:22px;font-weight:900;color:#fff">{company}</div>
    <div style="font-size:12px;color:rgba(255,255,255,.65);margin-top:4px">Background Verification Request</div>
  </td></tr>
  <tr><td style="padding:36px 40px">
    ...
    <p style="color:#475569;font-size:13.5px;line-height:1.9;margin:0 0 16px">
      Congratulations on accepting the offer for <b style="color:#1a56db">{role}</b> at <b>{company}</b>! 🎉
    </p>
    <p style="color:#475569;font-size:13px;line-height:1.8;margin:0 0 24px">
      As the next step in your onboarding journey, we need you to complete a <b>background verification</b>.
      This is a standard process for all new hires and helps us maintain a safe and trusted workplace.
    </p>
    <div style="background:#f0f4ff;border-radius:10px;padding:20px 24px;margin-bottom:28px;border-left:4px solid #1a56db">
      <div style="font-size:12px;font-weight:700;color:#0d1b3e;margin-bottom:10px;text-transform:uppercase;letter-spacing:.06em">What you'll need to provide</div>
      <div style="display:flex;flex-direction:column;gap:8px">
        {''.join(f'<div style="display:flex;align-items:center;gap:10px;font-size:13px;color:#475569"><span style="color:#10b981;font-size:16px">✓</span>{item}</div>' for item in ['Personal details (Name, Aadhaar, PAN)', 'Educational qualifications', 'Previous employment details (if experienced)'])}
      </div>
    </div>
    <div style="text-align:center;margin-bottom:24px">
      <a href="{bg_link}" style="display:inline-block;background:linear-gradient(135deg,#1a56db,#0ea5e9);color:#fff;
         padding:15px 40px;border-radius:10px;text-decoration:none;font-weight:700;
         font-size:15px;letter-spacing:.3px;font-family:Arial,sans-serif;
         box-shadow:0 4px 12px rgba(26,86,219,.3)">
        🛡️ Start Background Verification
      </a>
    </div>
    <p style="color:#94a3b8;font-size:11px;text-align:center;line-height:1.7">
      This link is unique to you. Please do not share it with others.<br>
      Complete the verification at your earliest convenience to proceed with onboarding.
    </p>
  </td></tr>
  <tr><td style="background:#f8faff;padding:14px 40px;text-align:center;border-top:1px solid #e2e8f0">
    <p style="color:#94a3b8;font-size:11px;margin:0">Sent via OfferFlow · {company} HR Portal</p>
...
def offer_response(cid, action):
    ...
    if not c:
        return "<div style='font-family:sans-serif;text-align:center;margin-top:80px'><h2>Invalid or expired link.</h2></div>", 404
    ...
    if action == 'accept':
        print(f"[DEBUG ACTION] accept hit for cid={cid}, current offer_status={c.get('offer_status')}")  # ← ADD THIS
        ...
        if not existing.get('bg_email_sent'):
            ...
            if success:
                ...
                save_verifs(verifs)
                print(f"[BG EMAIL] ✅ Sent successfully to {c['email']}")
            else:
                print(f"[BG EMAIL] ❌ Failed to send to {c['email']}")
        else:
            print(f"[BG EMAIL SKIP] Already sent for vid={existing['id']}")
...
def background_verification_page(vid):
    ...
    v = verifs.get(vid)
    if not v:
        return "<div style='font-family:sans-serif;text-align:center;margin-top:80px'><h2>Invalid verification link.</h2></div>", 404
...
<table width="500" style="background:#fff;border-radius:16px;overflow:hidden">
  ...
  <tr><td style="padding:36px;text-align:center">
    ...
    <p style="color:#64748b;font-size:13px;line-height:1.8">
      We were unable to verify your employment history. We regret we cannot proceed at this time.
    </p>
  </td></tr>
</table></td></tr></table></body></html>""")
    save_verifs(verifs)
